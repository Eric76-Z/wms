...
export default {
  ...
  setup(props) {
    ...
    const autocompletecfg1 = reactive({ target: "location", loadAll: [] });
    autocompletecfg1.loadAll.push(location.value);
    const autocompletecfg = toRaw(autocompletecfg1);
  ...
  mounted() {
    ...
    if (this.location.value !== undefined) {
      //有数据
      this.autocompletecfg.loadAll = this.location;
    } else {
      //没有请求数据
      console.log("重新加载数据");
